...
import { cloudSections } from "./cloudTopics";
...
const Cloud = () => {
  ...
  // ✅ Safe filtering (section + topic)
  let filteredSections = [];
  try {
    const query = searchQuery.toLowerCase();

    filteredSections = cloudSections
      .map((section) => {
    ...
    <Container className="my-5">
      <FadeScrollWrapper>
        <h2 className="mb-4 text-center fw-bold">
          Cloud Topics (Interview Friendly)
        </h2>
        ...
        <Row className="mb-4">
          <Col md={6}>
            <SearchFilter
              ...
              placeholder="Search cloud topic's by name..."
            />
          </Col>
          <Col md={6}>
            <Form.Select
      ...
      <FadeScrollWrapper>
        <Accordion alwaysOpen>
          {filteredSections.map((section, sectionIndex) => (
            <Accordion.Item
              eventKey={`cloud-section-${sectionIndex}`}
              key={`cloud-section-${sectionIndex}`}
            >
              <Accordion.Header>
